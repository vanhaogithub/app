...
public interface SalarySummaryRepository extends JpaRepository<SalarySummaryEntity, Long> {
	 ...
	 * Page<SalarySummaryEntity> findByMonth(@Param("month") Date month,
	 * Pageable pageable);
	 */
	...
	Double getSalaryByUserId(@Param("userid") Long userid, @Param("month") Date month);

	@Query(value = "SELECT * FROM salarysummary u WHERE u.month = :month and u.userid = :userid", nativeQuery = true)
